  ...
  export class ChessGameParser {
    ...
    static parsePgnMetadata(pgnContent: string, userId: string): ChessGame {
      const chessGame: ChessGame = {
        id: userId,  // Assign userId to id field, assuming id and userId are interchangeable in this context
        pgn: pgnContent,
        userId
      };
  
      // Split the PGN content into lines and remove empty lines
      const lines = pgnContent.split(/\r?\n/).filter(line => line.trim() !== "");
  
      // Loop through each line and parse it based on known PGN header fields
      lines.forEach((line) => {
        if (line.startsWith("[Event ")) {
          // You can add additional processing if necessary
